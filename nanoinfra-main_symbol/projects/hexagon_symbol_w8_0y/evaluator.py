...
from .tokenizer import (
    EOS_ID, PAD_ID, PROMPT_LENGTH, SEQUENCE_LENGTH, ANSWER_LENGTH, build_layout,
...
def generate_coefficients(system, words, *, sequence_len=SEQUENCE_LENGTH,
    ...
    if device.type != "cuda":
        raise RuntimeError("Model evaluation requires CUDA; no CPU fallback")
        ...
        for start in range(0, len(words), batch_size):
            ...
            with torch.autocast("cuda", dtype=torch.bfloat16):
                generated = autoregressive_generate(
                    system, prompts, types, max_new_tokens=ANSWER_LENGTH,
...
def metrics(predictions, truths) -> dict:
    ...
    counts = {"n_samples": len(truths), "n_invalid": 0, "n_exact": 0,
              "n_magnitude": 0, "n_sign": 0, "n_high": 0, "n_low": 0}
    for pred, truth in zip(predictions, truths, strict=True):
        ...
        counts["n_high"] += int(abs(pred) // 100 == abs(truth) // 100)
        counts["n_low"] += int(abs(pred) % 100 == abs(truth) % 100)
    ...
    return {**counts, "exact_accuracy": counts["n_exact"] / n,
            ...
            "high_accuracy": counts["n_high"] / n, "low_accuracy": counts["n_low"] / n}
...
def evaluate(system, dataset, rows: np.ndarray, *, batch_size=64, save_predictions=False) -> dict:
    ...
    result = {"split": dataset.split, "dataset": dataset.identity(),
              ...
              "decoding": "greedy_unconstrained_base100_fixed2", "sample_metrics": metrics(predictions, truths),
              "by_coefficient": by_coefficient, "by_magnitude": by_magnitude}
    result["rare_groups"] = {}
    for name, predicate in (("high_zero", lambda c: abs(c) < 100),
                            ("high_nonzero", lambda c: abs(c) >= 100),
                            ("coefficient_1920", lambda c: c == 1920)):
        positions = [i for i, c in enumerate(truths) if predicate(c)]
        result["rare_groups"][name] = (metrics([predictions[i] for i in positions], [truths[i] for i in positions])
                                       if positions else {"n_samples": 0, "exact_accuracy": None})
    if save_predictions:
