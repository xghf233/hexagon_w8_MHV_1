...
CHECKPOINT_VERSION = "hexagon-w8-0y-base100-fixed2-training-v1"
...
def code_hashes() -> dict:
    ...
    paths = [project / name for name in (
        ...
        "train.py", "eval_checkpoint.py", "data_contract.py", "convert_data.py", "runtime.py",
        "gpu_smoke.py")]
    paths += sorted((project / "configs").glob("*.yaml"))
...
def run_contract(config: dict, dataset, *, head_type: str) -> dict:
    return {"version": CHECKPOINT_VERSION, "dataset": dataset.identity(),
            ...
            "task": "hexagon_w8_0y_nonzero", "coefficient_scale": 32, "split_type": "random_row",
...
def read_metadata(directory: str | Path) -> dict:
    ...
    if meta.get("hexagon_contract", {}).get("version") != CHECKPOINT_VERSION:
        raise ValueError("Not a supported hexagon checkpoint")
    ...
    contract = meta["hexagon_contract"]
...
def save(directory: Path, system, optimizers, loader, contract: dict,
    ...
    try:
        save_checkpoint_dcp(str(staging), system, optimizers, {
            ...
            "hexagon_contract": contract, "rng_state": rng, "best": best,
...
def resume(directory, system, optimizers, loader, contract: dict) -> dict:
    ...
    if meta["hexagon_contract"] != contract:
