"""Hexagon assembly and checked runtime attention (not stored in tensor state)."""
...
def model_config(config: dict) -> GPTConfig:
    ...
    if geometry != MODEL_GEOMETRY:
        raise ValueError("This Hexagon version locks model geometry to 4/256/4/4")
...
def attach_attention(system) -> None:
    ...
    for block in system.trunk.blocks:
        block.attn._word_bidi_mask = block.attn._word_bidi_mask.to(device)


def validate_attention(system) -> None:
    device = next(system.parameters()).device
    if device.type != "cuda":
        raise RuntimeError("Runtime model/mask verification requires CUDA")
    if getattr(system, "attention_config", None) != ATTENTION_CONFIG:
        raise ValueError("Expected hexagon word-bidirectional attention over [1,9)")
    expected = word_bidirectional_mask(system.config.sequence_len, WORD_START, WORD_END).to(device)
    for block in system.trunk.blocks:
        ...
        if not torch.equal(mask.to(device), expected):
            raise ValueError("Runtime attention mask content differs from hexagon protocol")
...
def build_gpu_system(config: dict):
    ...
    # Import only after the Linux/CUDA gate; there is no CPU test/model path.
    ...
    if sum(p.numel() for p in system.parameters()) != EXPECTED_PARAMETERS:
        raise ValueError("Unexpected model parameter count; expected 3,205,376")
