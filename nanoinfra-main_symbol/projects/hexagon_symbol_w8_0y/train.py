...
def validate_config(config: dict) -> None:
    ...
    if config["sequence_len"] != 16:
        raise ValueError("Hexagon fixed-two-block recipe uses sequence_len=16")
...
def run_training(config: dict) -> dict:
    ...
    train = HexagonDataset(config["data_dir"], "train", expected_metadata_sha256=config["metadata_sha256"])
    ...
    if config["resume_from"]:
        ...
        if output == checkpoint_root or checkpoint_root in output.parents:
            raise ValueError("Do not write a resumed run inside an immutable checkpoint")
