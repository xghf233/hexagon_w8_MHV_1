"""Single-GPU hexagon runner; no local execution is implied by this module.
...
from .data_contract import outside_repository, separate_path
from .model import build_gpu_system, model_config
from .runtime import require_cuda_server
from .tokenizer import ANSWER_LENGTH, SEQUENCE_LENGTH


def validate_config(config: dict) -> None:
    allowed = {"data_dir", "metadata_sha256", "output_dir", "resume_from", "stop_after_steps", "sequence_len",
    ...
    for key in ("data_dir", "output_dir"):
        if not isinstance(config[key], str) or not config[key].strip():
            raise ValueError(f"{key} must be an explicit nonempty path")
    if config["resume_from"] is not None and (
            not isinstance(config["resume_from"], str) or not config["resume_from"].strip()):
        raise ValueError("resume_from must be null or an explicit checkpoint path")
    digest = config["metadata_sha256"]
    if not isinstance(digest, str) or len(digest) != 64 or any(c not in "0123456789abcdef" for c in digest):
        raise ValueError("metadata_sha256 must be the trusted lower-case SHA-256")
    ...
    if config["sequence_len"] != SEQUENCE_LENGTH:
        raise ValueError("Hexagon fixed-two-block recipe uses sequence_len=128")
    if type(config["total_batch_size"]) is not int or config["total_batch_size"] != config["device_batch_size"] * config["sequence_len"]:
    ...
    if config["compile"] is not False:
        raise ValueError("Compile is disabled for the first M2 version")
...
def fresh_output(path: str, data_dir: Path) -> Path:
    ...
    output = outside_repository(path)
...
def run_training(config: dict) -> dict:
    ...
    full_train = HexagonDataset(config["data_dir"], "train", expected_metadata_sha256=config["metadata_sha256"])
    train = full_train
    baselines = TrainBaselines(full_train)
    if config["overfit_n_samples"] is not None:
        train = train.training_subset(config["overfit_n_samples"], config["seed"])
    val = HexagonDataset(config["data_dir"], "val", expected_metadata_sha256=config["metadata_sha256"])
    ev = config["evaluation"]
    ...
    if config["resume_from"]:
        checkpoint_root = Path(config["resume_from"]).expanduser().resolve(strict=True)
        separate_path(output, checkpoint_root)
    system = build_gpu_system(config)
    from core.training.optim import build_optimizers
    random.seed(config["seed"])
    np.random.seed(config["seed"])
    loader = HexagonDataLoader(train, config["device_batch_size"], sequence_len=config["sequence_len"],
                              seed=config["seed"], device="cuda")
    optimizers = build_optimizers(system, config["optimizer"])
    groups = [group for opt in optimizers for group in opt.param_groups]
    if len(groups) != 3:
        raise ValueError("Expected three optimizer roles")
    for group, role in zip(groups, ("unembedding", "embedding", "matrix"), strict=True):
        group["role"] = role
    for group, configured in zip(groups, (config["optimizer"]["unembedding_lr"],
                                          config["optimizer"]["embedding_lr"],
                                          config["optimizer"]["lr_max"]), strict=True):
        if not math.isclose(group["initial_lr"], configured * math.sqrt(3), rel_tol=1e-12):
            raise ValueError("Core optimizer LR scaling differs from the M2 recipe")
        if group["eps"] != 1e-10:
            raise ValueError("Core AdamW epsilon differs from the locked recipe")
    ...
    write_json(output / "run.json", {
        ...
        "effective_optimizer_groups": [
            {"role": group["role"], "lr": group["initial_lr"], "eps": group["eps"], "weight_decay": group["weight_decay"]}
            for opt in optimizers for group in opt.param_groups],
        "samples_per_step": loader.batch_size,
        "training_rows": len(train), "frozen_train_pool_rows": len(full_train),
        "training_unique_inputs": len(set(map(int, train.input_group_ids[train.indices]))),
        "training_unique_targets": len(set(map(int, train.coefficients[train.indices]))),
        "planned_average_presentations": config["max_steps"] * loader.batch_size / len(train),
        "nominal_tokens_per_step": config["total_batch_size"],
        "actual_input_tokens_per_step": loader.batch_size * (SEQUENCE_LENGTH - 1),
        "supervised_tokens_per_step": loader.batch_size * ANSWER_LENGTH,
    })
    np.savez(output / "evaluation_rows.npz", val_subset=val_subset, train_diagnostic=train_rows,
             training_rows=train.indices)
    write_json(output / "baseline_val_full.json", baselines.evaluate(val, val.indices))
    ...
    while completed < end_step:
        ...
        if completed % config["logging"]["log_every"] == 0 or last or completed == initial_steps + 1:
            log({"event": "train", "completed_steps": completed, "loss": float(loss.detach()),
                 ...
                 "input_tokens": completed * loader.batch_size * (SEQUENCE_LENGTH - 1),
                 "supervised_tokens": completed * loader.batch_size * ANSWER_LENGTH,
                 "lr_groups": [group["lr"] for opt in optimizers for group in opt.param_groups],
                 "lr_roles": [group["role"] for opt in optimizers for group in opt.param_groups],
                 "session_elapsed_seconds": time.perf_counter() - started,
                 "peak_cuda_bytes": torch.cuda.max_memory_allocated(),
                 "session_rows_per_second": (completed - initial_steps) * loader.batch_size /
        ...
        if do_eval:
            ...
            try:
                ...
                log({"event": tag, "completed_steps": completed, **result["sample_metrics"],
                     "seen_input38": result["by_input_novelty"]["seen_input38"],
                     "unseen_input38": result["by_input_novelty"]["unseen_input38"],
                     "input_group_macro": result["input_group_macro"]})
    ...
    summary = {"completed_steps": completed, "rows_consumed": loader.state_dict()["rows_consumed"],
               ...
               "training_rows": len(train), "training_unique_inputs": len(set(map(int, train.input_group_ids[train.indices]))),
               "nominal_tokens": completed * config["total_batch_size"],
               "actual_input_tokens": completed * loader.batch_size * (SEQUENCE_LENGTH - 1),
               "supervised_tokens": completed * loader.batch_size * ANSWER_LENGTH,
